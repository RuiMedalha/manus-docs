...
export async function importEmailAttachmentToInbox(input: { tenant: { id: number; folderPattern: string }; userId: number; filename: string; contentType: string; bytes: Buffer; sourceAddress: string | null; sourceSubject: string; }) {
  ...
  await recordAudit({ tenantId: input.tenant.id, actorUserId: input.userId, action: "outlook.attachment_imported", resourceType: "document", resourceId: String(document.id), metadata: { filename: input.filename, sourceAddress: input.sourceAddress, sourceSubject: input.sourceSubject, suggestedFolder, ocrJobId: ocrJob?.id ?? null } });
